...
public class EntityTheoreticalTsar extends ThrownEntity
{
	public int	ticksInAir	= 0;
	public int aoc = 0;
	public boolean hasTrollface;

    public EntityTheoreticalTsar(EntityType<? extends EntityTheoreticalTsar> type, World world) {
        super(type, world);
    }
    ...
    public void explode()
	{
		if (!getWorld().isClient)
		{
			TsarBomba tsar = new TsarBomba((int)getX(), (int)getY(), (int)getZ(), getWorld(), (int) ((RivalRebels.tsarBombaStrength + (aoc * aoc)) * 0.6f));
			EntityTheoreticalTsarBlast tsarblast = new EntityTheoreticalTsarBlast(getWorld(), (int)getX(), (int)getY(), (int)getZ(), tsar, RivalRebels.tsarBombaStrength + (aoc * aoc));
