...
public class EntityTachyonBomb extends ThrownEntity
{
	public int	ticksInAir	= 0;
	public int aoc = 0;
	public boolean hasTrollface;

    public EntityTachyonBomb(EntityType<? extends EntityTachyonBomb> type, World world) {
        super(type, world);
    }
	...
	public EntityTachyonBomb(World par1World, double x, double y, double z, float yaw, float pitch, int charges, boolean troll) {
		...
		aoc = charges;
		hasTrollface = troll;
		if (!RivalRebels.nukedrop && !par1World.isClient)
		{
			explode();
		}
	}

	public EntityTachyonBomb(World world, float px, float py, float pz, float f, float g, float h)
	{
		this(world);
    ...
    public void explode()
	{
		if (!getWorld().isClient)
		{
			TachyonBomb tsar = new TachyonBomb((int)getX(), (int)getY(), (int)getZ(), getWorld(), (int) ((RivalRebels.tsarBombaStrength + (aoc * aoc)) * 1.0f));
			EntityTachyonBombBlast tsarblast = new EntityTachyonBombBlast(getWorld(), (int)getX(), (int)getY(), (int)getZ(), tsar, RivalRebels.tsarBombaStrength + (aoc * aoc));
