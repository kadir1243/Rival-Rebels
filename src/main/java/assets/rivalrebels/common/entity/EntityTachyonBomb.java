...
public class EntityTachyonBomb extends ThrowableProjectile
{
	...
	public EntityTachyonBomb(Level par1World, double x, double y, double z, float yaw, float pitch, int charges, boolean troll) {
		...
		aoc = charges;
		hasTrollface = troll;
		if (!RivalRebels.nukedrop && !par1World.isClientSide)
		{
			explode();
		}
	}
    ...
    public void explode()
	{
		if (!level().isClientSide)
		{
			TachyonBomb tsar = new TachyonBomb((int)getX(), (int)getY(), (int)getZ(), level(), (int) ((RivalRebels.tsarBombaStrength + (aoc * aoc)) * 1.0f));
			EntityTachyonBombBlast tsarblast = new EntityTachyonBombBlast(level(), (int)getX(), (int)getY(), (int)getZ(), tsar, RivalRebels.tsarBombaStrength + (aoc * aoc));
