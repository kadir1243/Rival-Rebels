...
public class EntityRoddiskLeader extends EntityInanimate
{
	...
	public EntityRoddiskLeader(World par1World, Entity par2EntityLiving, float par3)
	{
		super(par1World);
	...
	public void onUpdate()
	{
		...
		if (var3 != null)
		{
			...
			if (var3.entityHit != null)
			{
				...
				if (var3.entityHit instanceof EntityPlayer entityPlayerHit && shooter instanceof EntityPlayer && var3.entityHit != shooter) {
                    for (ItemStack armorStack : entityPlayerHit.inventory.armorInventory) {
                        if (!armorStack.isEmpty()) {
							armorStack.damageItem(30, entityPlayerHit);
                ...
                {
                    RivalRebels.flare.onPlayerDestroy(world, var3.getBlockPos(), state);
                }
                else if (state.getBlock() == RivalRebels.landmine || state.getBlock() == RivalRebels.alandmine)
                {
                    RivalRebels.landmine.onEntityCollision(world, var3.getBlockPos(), state, this);
                }
                else
                {
                    ...
                    if (var3.sideHit == EnumFacing.EAST || var3.sideHit == EnumFacing.WEST) this.motionX *= -1;
                    if (var3.sideHit == EnumFacing.UP || var3.sideHit == EnumFacing.DOWN) this.motionY *= -1;
