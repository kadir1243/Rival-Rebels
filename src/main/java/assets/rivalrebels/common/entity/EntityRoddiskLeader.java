...
public class EntityRoddiskLeader extends RoddiskBase {
    public EntityRoddiskLeader(EntityType<? extends EntityRoddiskLeader> type, World par1World)
    {
	...
	public EntityRoddiskLeader(World par1World, LivingEntity shooter, float par3) {
		...
		if (var3 != null)
		{
			...
			if (var3.getType() == HitResult.Type.ENTITY)
			{
				...
				if (entityHit instanceof PlayerEntity entityPlayerHit && shooter instanceof PlayerEntity && entityHit != shooter) {
                    for (ItemStack armorStack : entityPlayerHit.getInventory().armor) {
                        if (!armorStack.isEmpty()) {
							armorStack.damage(30, entityPlayerHit, player -> {});
				...
				{
					entityHit.damage(RivalRebelsDamageSource.tron, 15);
					if (entityHit instanceof SkeletonEntity)
					{
						entityHit.kill();
						this.world.spawnEntity(new EntityGore(world, entityHit, 0, 3));
						this.world.spawnEntity(new EntityGore(world, entityHit, 1, 3));
						this.world.spawnEntity(new EntityGore(world, entityHit, 2, 3));
						this.world.spawnEntity(new EntityGore(world, entityHit, 2, 3));
						this.world.spawnEntity(new EntityGore(world, entityHit, 3, 3));
						this.world.spawnEntity(new EntityGore(world, entityHit, 3, 3));
					}
					if (entityHit instanceof ZombieEntity && !(entityHit instanceof ZombifiedPiglinEntity))
					{
						entityHit.kill();
						this.world.spawnEntity(new EntityGore(world, entityHit, 0, 1));
						this.world.spawnEntity(new EntityGore(world, entityHit, 1, 1));
						this.world.spawnEntity(new EntityGore(world, entityHit, 2, 1));
						this.world.spawnEntity(new EntityGore(world, entityHit, 2, 1));
						this.world.spawnEntity(new EntityGore(world, entityHit, 3, 1));
						this.world.spawnEntity(new EntityGore(world, entityHit, 3, 1));
					}
				}
			}
