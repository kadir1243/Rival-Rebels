...
public class EntityFlameBallGreen extends EntityInanimate
{
	...
	public EntityFlameBallGreen(World par1World, EntityPlayer player, float par3)
	{
		super(par1World);
		setSize(0.5F, 0.5F);
			...
			for (int x = -3; x < 4; x++)
			{
				for (int y = -1; y < 2; y++)
				{
					for (int z = -3; z < 4; z++)
					{
                        ...
                        Block id = worldObj.getBlockState(pos).getBlock();
						if (id == Blocks.air || id == Blocks.snow || id == Blocks.ice || id == Blocks.leaves || id == Blocks.leaves2) worldObj.setBlockState(pos, Blocks.fire.getDefaultState());
					}
				}
			}
		}
