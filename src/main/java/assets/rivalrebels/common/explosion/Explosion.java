...
public class Explosion
{
				...
				for (int Z = -tworadius; Z <= tworadius; Z++)
				{
					...
					if (block != null && block != Blocks.air && block != Blocks.bedrock)
					{
						...
						else if (dist < radius)
						{
							...
							if (dist < halfradius)
							{
								breakBlock(world, xx, yy, zz, x, y, z);
							}
							else if (varrand > 0)
							{
								if ((rand.nextInt(varrand) == 0 || rand.nextInt(varrand / 2 + 1) == 0))
	...
	private void breakBlock(World world, int xx, int yy, int zz, double x, double y, double z)
	{
        BlockPos pos = new BlockPos(xx, yy, zz);
		...
		if (block == Blocks.oak_stairs || block == Blocks.acacia_stairs || block == Blocks.dark_oak_stairs || block == Blocks.birch_stairs) world.setBlockState(pos, Blocks.planks.getDefaultState());
