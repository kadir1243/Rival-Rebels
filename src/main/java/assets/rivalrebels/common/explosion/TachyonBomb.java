...
public class TachyonBomb
{
	...
	private boolean processChunk(int x, int z)
	{
		...
		else if (dist <= radius * radius * 1.3125 * 1.3125)
		{
			...
			{
				IBlockState block = world.getBlockState(new BlockPos(x + posX, y, z + posZ));
				if (!block.isOpaqueCube()) world.setBlockToAir(new BlockPos(x + posX, y, z + posZ));
			}
			return true;
		}
		return false;
	...
	private int getTopBlock(int x, int z, double dist)
	{
		...
		for (int y = 256; y > 0; y--)
		{
			...
			if (block != Blocks.AIR)
			{
				...
				{
					world.setBlockToAir(new BlockPos(x, y, z));
					if (dist > radius / 2 && block == Blocks.LOG && world.getBlockState(new BlockPos(x, y - 1, z)) == Blocks.LOG) isTree = true;
					if (!found && isTree)
					{
						foundY = y;
						found = true;
