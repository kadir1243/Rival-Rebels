...
public class RenderTachyonBombBlast extends Render<EntityTachyonBombBlast>
{
	...
	public RenderTachyonBombBlast(RenderManager manager)
	{
        super(manager);
	...
	@Override
	public void doRender(EntityTachyonBombBlast tsar, double x, double y, double z, float var8, float var9)
	{
		tsar.time++;
		...
		else if (tsar.time < 600 && radius - RivalRebels.tsarBombaStrength > 9)
		{
			...
			GlStateManager.scale(RivalRebels.shroomScale*2.0f,RivalRebels.shroomScale,RivalRebels.shroomScale*2.0f);
			...
			GlStateManager.popMatrix();
			GlStateManager.pushMatrix();
			GlStateManager.translate(x, y + elev * 4, z);
			GlStateManager.scale(RivalRebels.shroomScale,RivalRebels.shroomScale*3.0f,RivalRebels.shroomScale);
			GlStateManager.rotate((float) (elev * -3), 1, 0, 0);
		...
		else
		{
			...
			GlStateManager.scale((float) (horizontal * radius * 0.116f), (float) (radius * 0.065f), (float) (horizontal * radius * 0.116f));
