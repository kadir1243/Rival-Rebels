...
public class RenderRhodesRightUpperLeg extends EntityRenderer<EntityRhodesRightUpperLeg>
{
    ...
    @Override
    public void render(EntityRhodesRightUpperLeg entity, float yaw, float tickDelta, PoseStack matrices, MultiBufferSource vertexConsumers, int light) {
		matrices.pushPose();
		...
		RenderRhodes.thigh.render(TEXTURE.buffer(vertexConsumers, RenderType::entitySolid), new Vector3f(colors[entity.getColor()*3], colors[entity.getColor()*3+1], colors[entity.getColor()*3+2]), light, OverlayTexture.NO_OVERLAY);
		matrices.popPose();
	}

    @Override
    public ResourceLocation getTextureLocation(EntityRhodesRightUpperLeg entity) {
