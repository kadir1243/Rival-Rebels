...
public class RenderRhodesHead extends EntityRenderer<EntityRhodesHead> {
    ...
    public void render(EntityRhodesHead entity, float yaw, float tickDelta, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light) {
        ...
        RenderRhodes.head.render(TEXTURE.getVertexConsumer(vertexConsumers, RenderLayer::getEntitySolid), new Vector3f(colors[entity.getColor()*3], colors[entity.getColor()*3+1], colors[entity.getColor()*3+2]), light, OverlayTexture.DEFAULT_UV);
        matrices.pop();
    }
