		...
		if (dist < radius * radius) {
			...
			if (dist < limit) {
				...
				for (int Y = ylimit; Y > ylimit - (world.random.nextInt(5) + 2); Y--) {
					...
					if (blockType == 1) world.setBlockAndUpdate(pos, RRBlocks.petrifiedstone1.get().defaultBlockState().setValue(BlockPetrifiedStone.META, metadata));
					else if (blockType == 2) world.setBlockAndUpdate(pos, RRBlocks.petrifiedstone2.get().defaultBlockState().setValue(BlockPetrifiedStone.META, metadata));
					else if (blockType == 3) world.setBlockAndUpdate(pos, RRBlocks.petrifiedstone3.get().defaultBlockState().setValue(BlockPetrifiedStone.META, metadata));
					else world.setBlockAndUpdate(pos, RRBlocks.petrifiedstone4.get().defaultBlockState().setValue(BlockPetrifiedStone.META, metadata));
				}
			}
