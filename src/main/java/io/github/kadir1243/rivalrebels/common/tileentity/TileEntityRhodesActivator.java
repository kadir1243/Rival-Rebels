...
public class TileEntityRhodesActivator extends TileEntityMachineBase {
	...
	public TileEntityRhodesActivator(BlockPos pos, BlockState state) {
        super(RRTileEntities.RHODES_ACTIVATOR.get(), pos, state);
        pInM = 40;
		pInR = 20;
		...
		if (!level.isClientSide())
		{
			if (charge == 100)
			{
				...
				for (int i = 0; i < 31*9; i++)
				{
					...
					if (b == 1 && (s1 != RRBlocks.conduit.get() || s2 != RRBlocks.conduit.get()))
					{
						buildrhodes = false;
						break;
					}
					if (b == 2 && (s1 != RRBlocks.rhodesactivator.get() || s2 != RRBlocks.rhodesactivator.get()))
					{
						buildrhodes = false;
						break;
					}
