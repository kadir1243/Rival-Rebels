...
public class EntityHotPotato extends ThrowableProjectile {
	...
	{
		this(RREntities.HOT_POTATO.get(), level);
	}
	...
	public void tick()
	{
		...
		if (tickCount < 2 && dorounds) {
            this.playSound(RRSounds.NUKE.get(), 100, 1);
            setDeltaMovement(Vec3.ZERO);
			setPos(nextx+0.5f, nexty+0.5f, nextz+0.5f);
			level().setBlockAndUpdate(new BlockPos(nextx, nexty-400, nextz), RRBlocks.jump.get().defaultBlockState());
            reapplyPosition();
			return;
		}
