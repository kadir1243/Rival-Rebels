...
public class EntityRoddiskOfficer extends RoddiskBase {
    ...
    {
        this(RREntities.RODDISK_OFFICER.get(), level);
    }

    public EntityRoddiskOfficer(Level world, Entity shooter, float speed) {
		super(RREntities.RODDISK_OFFICER.get(), world, shooter);
	}
	...
	public void tick() {
		...
		if (tickCount == 10)
		{
            this.playSound(RRSounds.RODDISK_UNKNOWN0.get());
		}

		int radius = 2;

        level().getEntities((Entity) null, new AABB(position().add(-radius - 1, radius + 1, -radius - 1), position().add(radius + 1, -radius - 1, radius + 1)), entity -> entity instanceof Arrow).forEach(entity -> entity.kill((ServerLevel) level()));
		...
		if (var3 != null)
		{
			...
			if (var3.getType() == HitResult.Type.ENTITY)
			{
				...
				else
				{
					...
					{
						hitEntity.kill((ServerLevel) level());
                ...
                {
                    state.getBlock().destroy(level(), pos, RRBlocks.flare.get().defaultBlockState());
                }
