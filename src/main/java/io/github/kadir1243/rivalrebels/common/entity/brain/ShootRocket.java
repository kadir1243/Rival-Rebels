...
public class ShootRocket extends Behavior<EntityRhodes> {
    public ShootRocket() {
        super(Map.of(MemoryModuleType.ATTACK_TARGET, MemoryStatus.REGISTERED,
            MemoryModuleTypes.ROCKET_BLOCK_TARGET.get(), MemoryStatus.REGISTERED));
    }
    ...
    protected void tick(ServerLevel level, EntityRhodes owner, long gameTime) {
        ...
        if (brain.getMemory(MemoryModuleTypes.ROCKET_BLOCK_TARGET.get()).filter(Predicate.not(BlockEntity::isRemoved)).isEmpty()) {
            brain.eraseMemory(MemoryModuleTypes.ROCKET_BLOCK_TARGET.get());
        }
    }
    ...
    private void shootRocketsAtBestTarget(EntityRhodes owner, float syaw, float cyaw) {
        ...
        BlockEntity targetBlock = owner.getBrain().getMemory(MemoryModuleTypes.ROCKET_BLOCK_TARGET.get()).orElse(null);
