...
public class CommandPassword {
    ...
    private static int execute(CommandSourceStack source, String code) {
        ...
        if (p.rrrank != rank || rank == RivalRebelsRank.REGULAR) {
            p.rrrank = rank;
            source.getPlayer().playSound(rank.sound.value());
