...
public class EntityFlameBall2 extends FlameBallProjectile {
	...
	public EntityFlameBall2(Level level) {
		this(RREntities.FLAME_BALL2.get(), level);
	}
    ...
    private void fire() {
		if (!level().isClientSide()) {
			...
			else if (state.is(BlockItemTags.COAL_ORES.block()) && random.nextInt(30) == 0) level().setBlockAndUpdate(blockPosition(), Blocks.LAVA.defaultBlockState());
			...
			else if (state.is(BlockItemTags.LAPIS_ORES.block()) && random.nextInt(30) == 0) level().setBlockAndUpdate(blockPosition(), Blocks.LAVA.defaultBlockState());
			...
			else if ((state.is(RRBlocks.camo1) || state.is(RRBlocks.camo2) || state.is(RRBlocks.camo3)) && random.nextInt(40) == 0) level().setBlockAndUpdate(blockPosition(), RRBlocks.steel.get().defaultBlockState());
