...
public class EntityRhodesPiece extends Entity {
    ...
    public static final EntityDataAccessor<Holder<RhodesType>> TYPE = SynchedEntityData.defineId(EntityRhodesPiece.class, RhodesTypes.HOLDER_DATA_SERIALIZER.get());
	...
	public void tick() {
		super.tick();
		if (random.nextInt(Math.max(getMaxAge()*(RRConfig.SERVER.isRhodesPromode() ?1:30) - tickCount, RRConfig.SERVER.isRhodesPromode()?100:1))==0 && !level().isClientSide()) {
    ...
    protected void readAdditionalSaveData(ValueInput valueInput) {
		health = valueInput.getDoubleOr("health", 0);
	}

    @Override
    protected void addAdditionalSaveData(ValueOutput valueOutput) {
        valueOutput.putDouble("health", health);
	}
}
