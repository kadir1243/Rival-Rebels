...
@OnlyIn(Dist.CLIENT)
public record LoaderRenderer(Identifier texture) implements NoDataSpecialModelRenderer {
    @Override
    public void submit(PoseStack poseStack, SubmitNodeCollector submitNodeCollector, int lightCoords, int overlayCoords, boolean hasFoil, int outlineColor) {
        ModelLoader.render(submitNodeCollector, RenderTypes.entitySolid(this.texture()), poseStack, 0, lightCoords, overlayCoords);
	}
    ...
    public record Unbaked(Identifier texture) implements NoDataSpecialModelRenderer.Unbaked {
        public static final Identifier ID = RRIdentifiers.create("loader_renderer");
        ...
        public SpecialModelRenderer<Void> bake(BakingContext context) {
            return new LoaderRenderer(texture);
        }
