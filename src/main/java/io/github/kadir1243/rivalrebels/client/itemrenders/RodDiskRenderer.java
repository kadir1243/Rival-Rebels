...
@OnlyIn(Dist.CLIENT)
public record RodDiskRenderer(Identifier texture) implements NoDataSpecialModelRenderer {
    @Override
    public void submit(PoseStack poseStack, SubmitNodeCollector submitNodeCollector, int lightCoords, int overlayCoords, boolean hasFoil, int outlineColor) {
		...
		ModelDisk.render(poseStack, submitNodeCollector, RenderTypes.entitySolid(texture), lightCoords, overlayCoords);
    ...
    public record Unbaked(Identifier texture) implements NoDataSpecialModelRenderer.Unbaked {
        public static final Identifier ID = RRIdentifiers.create("rod_disk_renderer");
        ...
        public SpecialModelRenderer<Void> bake(BakingContext context) {
            return new RodDiskRenderer(texture);
        }
